...
import { withTempDownloadPath, type ClawdbotConfig } from "./sdk-shim.js";
...
export async function sendMediaFeishu(params: {
  ...
  if (mediaBuffer) {
    ...
    if (ab.byteLength > mediaMaxBytes) throw new Error(`Media too large: ${ab.byteLength} > ${mediaMaxBytes}`);
    buffer = Buffer.from(ab);
    const urlPath = new URL(mediaUrl).pathname;
    name = fileName ?? urlPath.split("/").pop() ?? "file";
  } else {
