...
| 文件 | 用途 | 是否需要定制 |
|------|------|------------|
| `.cursor/SOUL.md` | AI 的灵魂和人格准则 | 可选（默认已有不错的通用人格） |
| `.cursor/IDENTITY.md` | 名字、Emoji、气质 | **推荐**（给你的 AI 一个身份） |
| `.cursor/USER.md` | 你的个人信息和偏好 | **推荐**（帮 AI 更好地服务你） |
...
| `.cursor/TOOLS.md` | Cursor 原生能力清单 + 自定义工具备忘 | 按需添加你的特定工具信息 |

### 生命周期

| 文件 | 用途 | 是否需要定制 |
|------|------|------------|
| `.cursor/BOOTSTRAP.md` | 出生仪式（首次启动时引导 AI 建立身份，完成后自动删除） | 一般不用改 |
| `.cursor/BOOT.md` | 启动自检清单（每次服务启动时执行一次） | 可选（添加你的启动检查项） |
...
| `.cursor/HEARTBEAT.md` | 心跳检查清单 | AI 自动管理，也可手动编辑 |
| `.cursor/TASKS.md` | 定时任务说明 | 参考文档 |
...
3. 首次对话时 AI 会通过 `.cursor/BOOTSTRAP.md` 引导你完成个性化（出生仪式）
4. 也可以手动编辑 `.cursor/IDENTITY.md` 和 `.cursor/USER.md` 填入你的个人信息
5. `.cursor/MEMORY.md` 和 `.cursor/memory/*.md` 会随着使用自动积累
...
- **双层记忆**: `.cursor/MEMORY.md`（长期精华） + `.cursor/memory/YYYY-MM-DD.md`（每日日记）
- **向量搜索**: 语义相似度检索（需配置嵌入 API）
- **记忆召回**: 回答关于过去的问题前必须先搜索记忆
- **记忆防丢失**: 长对话中主动保存关键信息，防止上下文溢出
- **禁止心理笔记**: 强制文件持久化，杜绝"我会记住"的幻觉
- **会话日志**: `.cursor/sessions/*.jsonl` 记录所有对话
- **心跳状态**: `.cursor/memory/heartbeat-state.json` 追踪检查历史
...
所有文件都可以自由编辑。AI 也会根据交互自动更新 `.cursor/MEMORY.md` 和日记文件。

`.cursor/SOUL.md` 是灵魂文件——如果 AI 要修改它，它会先告诉你。
